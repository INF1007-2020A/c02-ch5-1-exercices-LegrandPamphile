...
def format_number(number, num_decimal_digits):
	#Séparer ls entières et décimales
	decimal_part = abs(number) % 1.0
	whole_part = int(abs(number))

	#Formater la partie décimale
	#C'est magique : decimal_str = f"{decimal_part :.{num_decimal_digits}f}"[1:]
	decimal_part = str(round(decimal_part * 10**num_decimal_digits))
	...
	return whole_part_str + decimal_part
...
def get_triangle(num_rows):
	...
	#Construire la première et dernière ligne
